...
var (
	...
	OutMulticastName = make(chan string, 100) //广播需要同步的域名
...
func AddBuildName(name string) (ok bool) {
	...
	for _, one := range interval {
		addBuildTempName(name, one)
	}
	addBuildTempNameRemove(name, now+60)
	return
}
...
*/
func AgreeTempName(name, replyMd5 string) {
...
func AddForeverName(name *Name) {
	...
	foreverNameLock.Unlock()
	now := time.Now().Unix()
	AddSyncMulticastName(name.Name, now+config.Time_name_sync_multicast)
}
...
/*
	添加一个临时id
*/
func (this *Name) AddId(sid, pid *utils.Multihash) bool {
