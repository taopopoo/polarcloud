...
type f func(class, params string)
...
/*
	...
	@params  string   字符串参数
*/
func (this *Task) Add(tick int64, class, params string) {
	//	go fmt.Println("add")
...
func (this *Task) Remove(tick int64, class, params string) {
	this.lock.Lock()
	ticks, ok := this.taskMap[tick]
	if ok {
		ticks.lock.Lock()
		...
		for i, one := range ticks.ticks {
			if one.class == class && one.params == params {
				//这里可以优化内存
				ticks.ticks = append(ticks.ticks[:i], ticks.ticks[i+1:]...)
				break
			}
		}
		ticks.lock.Unlock()
...
func NewTick(tick int64, class, params string) *Tick {
	return &Tick{
		tick:   tick,
		class:  class,
		params: params,
