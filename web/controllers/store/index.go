...
func (this *Index) Index() {
	this.Ctx.Redirect(http.StatusMovedPermanently, "/view/")
...
func (fil fileinfoObjList) Less(i, j int) bool {
	return fil[i].FileInfo.Time > fil[j].FileInfo.Time
}
...
func (this *Index) GetList() {
	_, fileInfos := store.GetFileinfoToSelfAll()
...
func (this *Index) GetFile() {
	...
	//上传文件信息到网络中
	go store.UpNetFileinfo(fileinfo)
